...
export default function FeedbackForm() {
  ...
  useEffect(() => {
    ...
    if (sid) {
      ...
      if (s) { setStoreId(s.id); setStoreName(s.name); setLocation(s.afe || ""); setQrDetected(true); }
    }
  }, [params]);
  ...
  function goToDetails() {
    ...
    if (!Object.values(ratings).every(v => v > 0)) e.ratings = "Please rate all categories";
    setErrors(e);
  ...
  async function submit() {
    ...
    try {
      await addDoc(collection(db, "feedbacks"), {
        storeId, store: storeName, location, whoAreYou, ratings, overall, recommend,
        name: name.trim() || "Anonymous", phone: phone.trim(), email: email.trim(),
        birthday: birthday || null, anniversary: anniversary || null,
        improve: improve.trim(), avgRating: parseFloat(avgR) || 0,
        createdAt: serverTimestamp(), source: "kreamz-qr",
      });
      setStep(3);
    } catch (e) {
      console.error(e);
      setSubmitErr("Could not save. Please check your connection and try again.");
  ...
  return (
    <>
      ...
      <div className="ff-bg">
            ...
            <div className="ff-step">
              ...
              {!qrDetected && (
                <div className="ff-field">
                  ...
                  <select className={`ff-input ff-sel${errors.store ? " err" : ""}`} value={storeId}
                    onChange={e => { setStoreId(e.target.value); const s = STORES.find(x => x.id === e.target.value); setStoreName(s ? s.name : ""); setLocation(s ? s.afe : ""); setErrors(p => ({ ...p, store: "", location: "" })); }}>
              ...
              <div className="ff-field">
                ...
                <select className={`ff-input ff-sel${errors.location ? " err" : ""}`} value={location}
              ...
              {RATINGS.map(q => (
                <div key={q.id} className="ff-rating-block">
                  ...
                  <div className="ff-emoji-row">
                    {EMOJIS.map(em => (
                      <button key={em.v} className={`ff-emoji-btn${ratings[q.id] === em.v ? " sel" : ""}`}
                        onClick={() => { setRatings(p => ({ ...p, [q.id]: em.v })); setErrors(p => ({ ...p, ratings: "" })); }} title={em.l}>
              ...
              <div className="ff-range-wrap">
                <label className="ff-label">Overall Score: <strong style={{ color: "#e91e8c" }}>{overall}/10</strong></label>
                <input type="range" min="1" max="10" value={overall} style={{ "--v": overall * 10 }} onChange={e => setOverall(+e.target.value)} className="ff-range" />
                <div className="ff-range-labels"><span>1 — Poor</span><span>10 — Perfect</span></div>
              </div>
              <button className="ff-btn" style={{ marginTop: "1.25rem" }} onClick={goToDetails}>Next →</button>
            </div>
          )}
            ...
            <div className="ff-step">
              ...
              <div className="ff-field">
                ...
                <div className="ff-who-grid">
                  {WHO_OPTIONS.map(opt => (
                    <button key={opt} className={`ff-who-btn${whoAreYou === opt ? " sel" : ""}`}
                      onClick={() => { setWhoAreYou(opt); setErrors(p => ({ ...p, whoAreYou: "" })); }}>
                      {opt === "Customer" && "🛍️ "}{opt === "Internal Team" && "👷 "}{opt === "Franchise Owner" && "🏪 "}{opt === "Store Member" && "👤 "}{opt}
                    </button>
                  ))}
                </div>
              ...
              <div className="ff-field">
                ...
                <input className="ff-input" type="email" placeholder="you@email.com" value={email} onChange={e => setEmail(e.target.value)} />
              </div>
              <div className="ff-field">
                <label className="ff-label">What Can We Improve? <span className="ff-req">*</span></label>
                <textarea className={`ff-input ff-textarea${errors.improve ? " err" : ""}`} rows={3}
                  placeholder="Share anything we can do better — taste, packaging, service, speed..."
                  value={improve} onChange={e => { setImprove(e.target.value); setErrors(p => ({ ...p, improve: "" })); }} />
              ...
              <div className="ff-field">
                ...
                <div className="ff-rec-row">
                  {["Yes, definitely! 🥰", "Maybe 🤔", "No 😔"].map((opt, i) => (
                    <button key={opt} className={`ff-rec-btn${recommend === i ? " sel" : ""}`} onClick={() => setRecommend(i)}>{opt}</button>
                  ))}
                </div>
              </div>
        ...
        <footer className="ff-footer">Made with 💝 by Kreamz India &bull; {STORES.length} outlets across India</footer>
      </div>
    </>
  );
}
