...
const C = {
  ...
  divider:  { borderTop:"1px solid rgba(255,255,255,.06)", margin:"12px 0" },
...
function WhoBadge({ who }) {
  const map = {
    "Customer":       C.badgeG,
    "Internal Team":  C.badgeB,
    "Franchise Owner":C.badge,
    "Store Member":   C.badgeY,
  };
  ...
  const filtered = useMemo(() => feedbacks.filter(f => {
    ...
    if (fSearch) {
      ...
      if (!(f.name?.toLowerCase().includes(q) || f.store?.toLowerCase().includes(q) ||
            f.email?.toLowerCase().includes(q) || f.comment?.toLowerCase().includes(q) ||
            f.improve?.toLowerCase().includes(q))) return false;
    }
    return true;
  ...
  return (
    <div style={C.wrap}>
      ...
      <aside style={C.sidebar}>
        ...
        <nav style={C.nav}>
          {TABS.map(t => (
            <button key={t.id} style={{ ...C.nb, ...(tab === t.id ? C.nba : {}) }}
              onClick={() => setTab(t.id)}>
              {t.icon} {t.label}
            </button>
          ))}
        ...
        <div style={C.sfooter}>
          ...
          <div style={{ fontSize:10.5, color:"rgba(255,255,255,.22)", marginBottom:2 }}>
        ...
        {fbErr && (
          <div style={C.errbox}>
            ...
            <div style={{ fontSize:12, marginTop:6, opacity:.75 }}>
              Fix: Firebase Console → Firestore → Rules → set <code style={{ background:"rgba(255,255,255,.1)", padding:"1px 5px", borderRadius:4 }}>allow read, write: if true;</code> → Publish
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div style={C.card}>
              ...
              {loading ? <div style={C.loading}>Connecting to Firebase…</div> :
               storeLB.length === 0 ? <div style={C.empty}>No feedbacks yet — share QR codes!</div> : (
                <div style={{ overflowX:"auto" }}>
                  <table style={C.tbl}>
                    <thead><tr>
                      {["#","Outlet","Location","Feedbacks","Avg /10"].map(h => <th key={h} style={C.th}>{h}</th>)}
                    </tr></thead>
                    <tbody>
                      {storeLB.map((s, i) => (
                        <tr key={s.n}>
        ...
        {tab === "feedbacks" && (
          <div>
            <div style={{ ...C.ph, display:"flex", justifyContent:"space-between", alignItems:"flex-end", flexWrap:"wrap", gap:10 }}>
              ...
              <button style={C.btn}
                onClick={() => exportToExcel(filtered, `kreamz-feedbacks-${new Date().toISOString().slice(0,10)}.xlsx`)}>
                ⬇ Export Excel
              </button>
            </div>
            ...
            {/* Filters */}
            <div style={C.frow}>
              <input style={C.srch} type="text" placeholder="🔍 Search name, outlet, email, feedback..."
              ...
              {(fSearch||fWho!=="All"||fStore!=="All"||fLocation!=="All"||fDays!=="all") && (
                <button style={C.btnGhost}
                  onClick={() => { setFSearch(""); setFWho("All"); setFStore("All"); setFLocation("All"); setFDays("all"); }}>
                  ✕ Clear
                </button>
              )}
            </div>
            ...
            {loading ? <div style={C.loading}>Loading…</div> :
             filtered.length === 0 ? <div style={C.empty}>No feedbacks match your filters.</div> :
             filtered.map(f => (
          ...
          <div>
            <div style={{ ...C.ph, display:"flex", justifyContent:"space-between", alignItems:"flex-end", flexWrap:"wrap", gap:10 }}>
              ...
              <button style={C.btn} onClick={() => {
                const data = feedbacks.filter(f => f.email || f.phone || f.birthday || f.anniversary);
                exportToExcel(data, "kreamz-customers.xlsx");
              }}>⬇ Export Customers</button>
            </div>
            {loading ? <div style={C.loading}>Loading…</div> : (
              <div style={C.card}>
                <div style={C.ctitle}>
                  🎂 Customers with Special Dates &nbsp;
                  <span style={{ ...C.badge, fontSize:11 }}>{feedbacks.filter(f => f.birthday||f.anniversary).length}</span>
                </div>
                <div style={{ overflowX:"auto" }}>
                  <table style={C.tbl}>
                    <thead><tr>
                      {["Name","Type","Phone","Email","Birthday","Anniversary","Outlet","Location"].map(h => (
                  ...
                  {feedbacks.filter(f => f.email||f.birthday||f.anniversary).length === 0 && (
                    <div style={C.empty}>No customer details collected yet.</div>
                  )}
                </div>
              </div>
            )}
        ...
        {tab === "export" && (
          <div>
            <div style={C.ph}>
              ...
              <p style={C.psub}>Download feedback data as professionally formatted Excel files</p>
            </div>
            ...
            <div style={C.card}>
              ...
              <div style={C.frow}>
                ...
                {(fWho!=="All"||fStore!=="All"||fLocation!=="All"||fDays!=="all") && (
                  <button style={C.btnGhost}
                    onClick={() => { setFWho("All"); setFStore("All"); setFLocation("All"); setFDays("all"); }}>
                    ✕ Clear Filters
                  </button>
                )}
              </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))", gap:12 }}>
              {[
                {
                  icon:"📊", title:"All Feedbacks (Filtered)", desc:`Export ${filtered.length} feedback records with all fields`,
                  fn: () => exportToExcel(filtered, `kreamz-feedbacks-${new Date().toISOString().slice(0,10)}.xlsx`)
                },
                {
                  icon:"📋", title:"All Feedbacks (Complete)", desc:`Export all ${feedbacks.length} feedback records`,
                  fn: () => exportToExcel(feedbacks, `kreamz-all-feedbacks-${new Date().toISOString().slice(0,10)}.xlsx`)
                },
                {
                  icon:"🎂", title:"Customers Database", desc:"Names, emails, phones, birthdays, anniversaries",
                  fn: () => exportToExcel(feedbacks.filter(f => f.email||f.phone||f.birthday||f.anniversary), "kreamz-customers.xlsx")
                },
                {
                  icon:"👥", title:"Customers Only", desc:"Only feedbacks from 'Customer' type",
                  fn: () => exportToExcel(feedbacks.filter(f => f.whoAreYou === "Customer"), "kreamz-customers-only.xlsx")
                },
                {
                  icon:"🏪", title:"Franchise & Store", desc:"Internal team, franchise owners, store members",
                  fn: () => exportToExcel(feedbacks.filter(f => f.whoAreYou !== "Customer"), "kreamz-internal.xlsx")
                },
              ].map((btn, i) => (
                <div key={i} style={{ ...C.card, marginBottom:0, cursor:"pointer", transition:"all .18s", border:"1px solid rgba(233,30,140,.15)" }}
                  onClick={btn.fn}
                ...
                >
                  ...
                  <button style={C.btn} onClick={e => { e.stopPropagation(); btn.fn(); }}>
                    ⬇ Download .xlsx
                  </button>
                </div>
              ))}
            ...
            <div style={{ ...C.card, marginTop:14 }}>
              <div style={C.ctitle}>👁️ Preview — First 5 Records</div>
              {filtered.slice(0, 5).map(f => (
                <div key={f.id} style={{ padding:"8px 0", borderBottom:"1px solid rgba(255,255,255,.05)", fontSize:12.5, color:"rgba(255,255,255,.6)", display:"flex", gap:10, flexWrap:"wrap" }}>
                  <span style={{ color:"white", fontWeight:600 }}>{f.name||"Anon"}</span>
                  <WhoBadge who={f.whoAreYou} />
                  <span>📍 {f.store}</span>
                  {f.email && <span>📧 {f.email}</span>}
                  <span style={{ color:"#e91e8c" }}>⭐ {f.overall}/10</span>
                  <span style={{ marginLeft:"auto", color:"rgba(255,255,255,.3)" }}>{fmtDate(f.createdAt)}</span>
                </div>
              ))}
              {filtered.length === 0 && <div style={C.empty}>No data to preview.</div>}
            </div>
          </div>
        )}
      </main>
